...
use crate::callback_param_rename::{is_param_rename_pair, maybe_apply_param_rename};
...
use crate::helper_array_unwrap::apply_helper_array_unwrap;
use crate::helper_call_site_substitution::{
    apply_helper_call_site_substitution, is_helper_call_site_alias_pair,
};
use crate::helper_zero_arg_alias::{apply_helper_zero_arg_alias, is_helper_zero_arg_alias_pair};
...
pub(super) use crate::walk_ctx::{
    CacheAlias, CatchNarrowedLocal, HelperCallSiteAlias, HelperZeroArgAlias, NarrowedRequestField,
    NonNullAliasLocal, ParamRename, Side, TransientLocal, WalkCtx,
};
...
pub(super) fn walk(ctx: &WalkCtx, base: Node, head: Node, findings: &mut Vec<Finding>) {
    ...
    if maybe_apply_param_rename(ctx, base, head, findings) {
        return;
    }
...
fn leaf_alias_consumed(ctx: &WalkCtx, base: Node, head: Node) -> bool {
    is_cache_alias_pair(ctx, base, head)
        ...
        || is_param_rename_pair(ctx, base, head)
}
...
fn block_rule_consumed(ctx: &WalkCtx, base: Node, head: Node, findings: &mut Vec<Finding>) -> bool {
    ...
    applied |= apply_helper_array_unwrap(&mut child_ctx, base, head);
    applied |= apply_helper_zero_arg_alias(&mut child_ctx, base, head);
    if applied {
